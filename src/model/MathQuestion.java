...
public class MathQuestion extends Question {
    ...
    public boolean checkAnswer(double answer) {
        if (Double.isInfinite(correctAnswer)) return Double.isInfinite(answer);
        if (Double.isNaN(correctAnswer))      return Double.isNaN(answer);

        double tolerance;
        switch (difficulty) {
            case EASY:   tolerance = 0.1;      break;
            case MEDIUM: tolerance = 0.01;     break;
            case HARD:   tolerance = 0.001;    break;
            case EXPERT: tolerance = 0.0001;   break;
